...
namespace spatial_temporal {
    ...
    Extractor::Extractor(ros::NodeHandle nh, std::string scanTopic, float windowDuration, std::string logFilename)
    ...
    Extractor::Extractor(ros::NodeHandle nh, std::string scanTopic, float windowDuration)
        : nh_(nh), scanTopic_(scanTopic), windowDuration_(windowDuration), onRunningWindow_(false), window_counter_(0), win_stamp_counter_(0)
    {
    ...
    void Extractor::initRosComunication()
    {
    ...
    sensor_msgs::PointCloud Extractor::laserMsgAsPointCloud(const sensor_msgs::LaserScan::ConstPtr &scanMsg)
    {
    ...
    void Extractor::publishAssembledCloud(ros::Time from, ros::Time to)
    {
        sensor_msgs::PointCloud cloud;
    ...
    void Extractor::publishAssembledCloud(sensor_msgs::PointCloud cloud){
        assembledCloudPublisher_.publish(cloud);
    }

    void Extractor::publishEigenvectorMarker(std_msgs::Header header, Eigen::VectorXd eigenvect,
    ...
    void Extractor::publishVariance(MinimalPointCloud &minimalCloud)
    {
    ...
    int Extractor::assembledCloud(ros::Time from, ros::Time to, sensor_msgs::PointCloud &cloud){
    ...
    void Extractor::setTemporalDimension(sensor_msgs::PointCloud &cloud, double time){
    ...
    void Extractor::saveToLogFile(std::string text){
            if (logFile_.is_open()){
            logFile_ << text.c_str() << std::endl << std::flush;
        }
    }

    void Extractor::saveCloudDataToLog(sensor_msgs::PointCloud &cloud){
        if (logFile_.is_open()){
            for (int i=0; i < cloud.points.size(); i++){
    ...
    void Extractor::legArrayCallback(const player_tracker::LegArray::ConstPtr &legArrayMsg){
        #pragma omp parallel for
    ...
    geometry_msgs::Point Extractor::computeClusterMean(sensor_msgs::PointCloud2 &input){
        sensor_msgs::PointCloud cloud;
    ...
    Eigen::MatrixXd Extractor::projectToEigenvector(Eigen::MatrixXd &dataset, Eigen::VectorXd &eigenvect){
        return eigenvect.transpose() * dataset;
    }

    Eigen::MatrixXd Extractor::cloudPointAsMatrix(sensor_msgs::PointCloud2 &input) {
        sensor_msgs::PointCloud cloud;
        if (sensor_msgs::convertPointCloud2ToPointCloud(input, cloud)){
    ...
    Eigen::MatrixXd Extractor::cloudPointAsMatrix(sensor_msgs::PointCloud &cloud) {
        Eigen::MatrixXd output(3, cloud.points.size());
        #pragma omp parallel for
        for (int i=0; i < cloud.points.size(); i++){
            output.col(i) << cloud.points[i].x, cloud.points[i].y, cloud.points[i].z;
    ...
    Eigen::MatrixXd Extractor::computeClusterVariance(Eigen::MatrixXd &input) {
    ...
    Eigen::MatrixXd Extractor::computeClusterVariance(sensor_msgs::PointCloud2 &input){
        sensor_msgs::PointCloud cloud;
        if (sensor_msgs::convertPointCloud2ToPointCloud(input, cloud)){
    ...
    Cloud2List Extractor::getClusters(sensor_msgs::PointCloud2 &input){
    ...
    Eigen::VectorXd Extractor::getClusterSmallestEigenvector(Eigen::MatrixXd cov){
    ...
    Eigen::VectorXd Extractor::getClusterSmallestEigenvector(sensor_msgs::PointCloud2 &cluster){
    ...
    Eigen::MatrixXd::Index Extractor::findSteepestEigenvector(Eigen::MatrixXd eigenvectors){
    ...
    void Extractor::saveProjectionToFile(){
    ...
    std::vector<std::pair <int,float>> Extractor::getSimilarity(sensor_msgs::PointCloud2 &cluster){
    ...
    float Extractor::computeJaccardSimilarity(sensor_msgs::PointCloud &cloudIn1, sensor_msgs::PointCloud &cloudIn2){
        // Sort lists
        // std::sort(cloudIn1.points.begin(), cloudIn1.points.end(), compPoints);
        // std::sort(cloudIn2.points.begin(), cloudIn2.points.end(), compPoints);
    ...
    void Extractor::publishClusterTextMarker(int marker_id, geometry_msgs::Point position){
    ...
    void Extractor::computeSpatialTemporalFeatures(){
    ...
    float Extractor::getAngleWithNormal(Eigen::VectorXd &eigenvect){
    ...
    Cloud2List Extractor::getClustersInWindow() {
    ...
    Eigen::MatrixXd Extractor::computeMinEigenvectors(Cloud2List &clusters){
        Eigen::MatrixXd eigenvects(3, clusters.size());      // Holds all eigenvectors;
        #pragma omp parallel for
        for(int i=0; i < clusters.size(); i++){
    ...
    Eigen::VectorXd Extractor::computeSteepestDirection(Cloud2List &clusters) {
        Eigen::MatrixXd eigenvects(3, clusters.size());      // Holds all eigenvectors;
        #pragma omp parallel for
        for(int i=0; i < clusters.size(); i++){
            ROS_DEBUG("Processing cluster %d", i);
    ...
    void Extractor::laserCallback(const sensor_msgs::LaserScan::ConstPtr &scanMsg){
    ...
    PointCloudList& Extractor::getWindowCloudList(){
        return cloudsInWindow;
    }

    void Extractor::clearWindowCloudList(){
        cloudsInWindow.clear();
    }

    void Extractor::clearCurrentClusterList(){
    ...
    void Extractor::appendWindowCloudList(sensor_msgs::PointCloud &cloud){
